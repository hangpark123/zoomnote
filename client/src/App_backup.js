...
function App() {
  ...
  const myWeekStatus = useMemo(() => {
    ...
    const map = Array.from({ length: maxWeeks }, (_, idx) => {
      const week = idx + 1;
      const has = myNotes.some((n) => n.report_year === calendarYear && n.report_week === week);
      const isVacation = myVacationWeeks.has(week);
      return { week, status: has ? 'done' : isVacation ? 'vacation' : 'missing' };
    });
    const todayWeek = getWeekNumber(new Date());
    return { list: map, currentWeek: todayWeek };
  ...
  const adminTargetUsers = useMemo(() => {
    ...
    return users.filter(
      (u) =>
        ['staff', 'leader', 'executive', 'master'].includes(u.role) &&
    ...
    <div className="app-root">
      <header className="app-header">
        ...
        <div className="app-header-meta">
           사용자: <span className="user-badge">{writerDisplay}</span>
        </div>
              ...
              <div className="card-body">
                ...
                <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(70px,1fr))', gap:6, marginTop:8}}>
                   {myWeekStatus.list.map((w) => (
                     ...
                     >
                       {`${w.week}주차\n${w.status==='done' ? '완료' : w.status==='vacation' ? '휴가' : '미작성'}`}
                     </button>
                   ))}
                 </div>
              </div>
            ...
            <div className="card-body">
                <div className="search-container">
                    <div className="search-wrapper">
                        ...
                        <input className="search-input" placeholder="검색어를 입력하세요" value={searchText} onChange={(e) => setSearchText(e.target.value)} />
                    </div>
                </div>
                {me?.role === 'master' && (
                  <div style={{border:'1px solid #e5e7eb', borderRadius:8, padding:12, marginBottom:12, display:'flex', flexWrap:'wrap', gap:12, alignItems:'center', background:'#f8fafc'}}>
                                      ...
                                      <td>
                                        <div style={{display:'flex', flexDirection:'column', alignItems:'flex-start', gap:4}}>
                                          <div style={{display:'flex', alignItems:'center', gap:'6px'}}>
                                            {note.checker_name ? (
                                              <>
                                                <span className="status-badge" style={{background:'#dcfce7', color:'#166534'}}>완료 {note.checker_name}</span>
                                              ...
                                              <>
                                                ...
                                                <button className="ghost-btn" style={{fontSize:'11px', color:'#2563eb'}} onClick={() => signNote(note.id, 'checker', false)}>{proxyMode ? '대리 서명' : '서명'}</button>
                                              </>
                                            )}
                                          </div>
                                          {note.checker_signed_at && (
                                            <div className="muted" style={{fontSize:11}}>{toDateTimeMinuteString(note.checker_signed_at)}</div>
                                          )}
                                        </div>
                                      </td>
                                      <td>
                                        <div style={{display:'flex', flexDirection:'column', alignItems:'flex-start', gap:4}}>
                                          <div style={{display:'flex', alignItems:'center', gap:'6px'}}>
                                            {note.reviewer_name ? (
                                              <>
                                                <span className="status-badge" style={{background:'#dcfce7', color:'#166534'}}>완료 {note.reviewer_name}</span>
                                              ...
                                              <>
                                                <span className="status-badge" style={{background:'#f1f5f9', color:'#475569'}}>대기</span>
                                                <button className="ghost-btn" style={{fontSize:'11px', color:'#2563eb'}} onClick={() => signNote(note.id, 'reviewer', false)}>{proxyMode ? '대리 서명' : '서명'}</button>
                                              </>
                                            )}
                                          </div>
                                          {note.reviewer_signed_at && (
                                            <div className="muted" style={{fontSize:11}}>{toDateTimeMinuteString(note.reviewer_signed_at)}</div>
                                          )}
                                        </div>
                                      </td>
                                      <td style={{textAlign:'right'}}>
            ...
            <div className="card-body" style={{display:'flex', flexDirection:'column', gap:8}}>
              ...
              {adminMissing.length > 0 && (
                <div style={{display:'flex', gap:8, flexWrap:'wrap'}}>
                  {adminMissing.map((u) => (
                    <span key={u.zoom_user_id} className="status-badge" style={{background:'#fef3c7', color:'#92400e'}}>
                      {u.name}
                    </span>
                  ))}
                </div>
