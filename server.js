...
// [Helper] Week Date Range (ISO Week)
function getWeekRange(year, week) {
  ...
  return {
    start: toDateStringSafe(monday),
    end: toDateStringSafe(friday)
  };
}
...
function toDateStringSafe(dateVal) {
  if (!dateVal) return '';
...
function formatSerialNo(year, week, seq) {
  // Format: YYYY-WW-NNN (e.g. 2026-05-001)
  return `${year}-${String(week).padStart(2, '0')}-${String(seq).padStart(3, '0')}`;
}
...
async function sendExport(ids, res) {
  ...
  for (const note of rows) {
    ...
    const safeName = `${note.writer_name || '작성자'}_${note.report_year}년_${note.report_week}주차.pdf`.replace(/[\\/:*?"<>|]/g, '_');
...
app.get('/api/research-notes', attachMe, async (req, res) => {
  try {
    ...
    sql += ' ORDER BY rn.serial_no DESC';
...
// [Helper] Serial Logic
// [Helper] Serial Logic
async function findNextSerialSeq({ reportYear, reportWeek }) {
  // Logic: Serial = Count of ALL notes in this year/week + 1 (Global Sequence for Week)
  // Reverted from per-user to global per user request ("Same 1" issue)
  const [rows] = await pool.query(
...
app.post('/api/research-notes', attachMe, conditionalUpload, async (req, res) => {
  try {
    ...
    // 문서 번호 자동 생성 (주차 기반 + 순서)
    const { nextSeq } = await findNextSerialSeq({
      reportYear: yr,
      reportWeek: wk,
      zoomUserId: me.zoom_user_id
    });

    console.log(
      `[ResearchNote] Serial Logic: Year=${yr}, Week=${wk}, User=${me.zoom_user_id} => Seq=${nextSeq}`
    );

    const generatedSerialNo = formatSerialNo(yr, wk, nextSeq);
...
app.get('/api/vacations', attachMe, async (req, res) => {
  try {
    ...
    // [Fix] Query 'research_notes' for vacations (title='휴가')
    const [rows] = await pool.query(
      `SELECT writer_zoom_user_id AS zoom_user_id, report_year AS year, report_week AS week, '휴가' AS reason, record_date AS created_at
       FROM research_notes
       WHERE report_year = ? AND report_week = ? AND title = '휴가'`,
      [year, week]
    );
    res.json(rows);
  } catch (e) {
...
app.get('/api/my-vacations', attachMe, async (req, res) => {
  try {
    ...
    // [Fix] Query 'research_notes' for vacations (title='휴가')
    const [rows] = await pool.query(
      `SELECT report_year AS year, report_week AS week, '휴가' AS reason, record_date AS created_at
       FROM research_notes
       WHERE writer_zoom_user_id = ? AND report_year = ? AND title = '휴가'
...
// [관리자] 휴가 처리 (자동 문서 생성/삭제)
app.post('/api/vacations', attachMe, async (req, res) => {
  try {
    if (req.me.role !== 'master') return res.status(403).json({ error: '권한이 없습니다.' });
    const { year, week, zoom_user_id, isVacation } = req.body;

    if (!year || !week || !zoom_user_id) {
      return res.status(400).json({ error: '잘못된 요청입니다.' });
    }

    if (isVacation) {
      // 1. Create Vacation Note
      const [exists] = await pool.query(
        'SELECT id FROM research_notes WHERE writer_zoom_user_id = ? AND report_year = ? AND report_week = ?',
        [zoom_user_id, year, week]
      );

      if (exists.length === 0) {
        // [Modified] Serial No Logic for Vacation (Use simple placeholder or generate real one)
        // Since vacation is a note, it should follow the same serial logic OR just a unique one.
        // User wants per-person sequence.
        // Let's reuse findNextSerialSeq which we will update shortly to be per-person.
        const { nextSeq } = await findNextSerialSeq({ reportYear: year, reportWeek: week, zoomUserId: zoom_user_id });
        const serialNo = formatSerialNo(year, week, nextSeq); // Updated Format

        const { start, end } = getWeekRange(year, week);
        const recordDate = start; // Record date as Monday

        await pool.query(
          `INSERT INTO research_notes 
           (writer_zoom_user_id, record_date, report_year, report_week, serial_no, title, period_start, period_end, weekly_goal, content)
           VALUES (?, ?, ?, ?, ?, '휴가', ?, ?, '휴가', '휴가')`,
          [zoom_user_id, recordDate, year, week, serialNo, start, end]
        );
      }
    } else {
      // 2. Delete Vacation Note (ONLY if title is '휴가')
      await pool.query(
        'DELETE FROM research_notes WHERE writer_zoom_user_id = ? AND report_year = ? AND report_week = ? AND title = ?',
        [zoom_user_id, year, week, '휴가']
      );
    }

    res.json({ ok: true });
  } catch (e) {
    console.error('POST /api/vacations error', e);
    res.status(500).json({ error: '휴가 처리 실패' });
  }
});
...
// DB 테이블 초기화 후 서버 시작
Promise.all([ensureZoomOauthTokenTable()]).then(() => {
  const server = app.listen(PORT, () => console.log(`Server running on port ${PORT}`));
  server.setTimeout(600000); // 10분 타임아웃 설정 (PDF 대량 다운로드 대응)
}).catch(err => {
